...
\begin{tikzpicture}
    ...
    \node (src_code) at (0, 16) [align=center]
    {program ``$p(x)$'' \\ source code};
    \node (paradox_maker) at (0, 15) [draw, align=center, shape=trapezium, shape border rotate=180]
	{Paradox Maker};
    \node (halt_question) at (0, 14) [draw, align=center, shape=ellipse]
    {``Does $p(p)$ halt?''};
    
    \node (inf_loop) at (-1, 12) [draw, align=center, shape=ellipse]
    {Infinite Loop};
    \node (halt) at (2, 12) [draw, align=center, shape=ellipse]
    {Halt};

    \draw[->, thick] (paradox_maker) -- (halt_question);
    \draw[->, thick] (halt_question) -- (inf_loop) node[midway, right] {Yes};
    \draw[->, thick] (halt_question) -- (halt) node[midway, right] {No};
\end{tikzpicture}
